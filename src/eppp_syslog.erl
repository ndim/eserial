...
%%%----------------------------------------------------------------------
%%% Created : 16 Feb 1999 by tobbe@serc.rmit.edu.au
%%% Function: General syslog framework.
%%%----------------------------------------------------------------------
-vc('$Id: eppp_syslog.erl,v 1.1.1.1 1999/09/05 09:54:20 tobbe Exp $ ').

-behaviour(gen_event).

%% External exports
-export([add_handler/0,add_handler/1,add_handler/2,error/1,error/2,
	 info/1,info/2,dbg/1,dbg/2,log/3,log/5]).
...
-define(WHO, eppp).        % Under what name syslog should log the info.
...
dbg(Msg)        -> log(debug(),Msg,[]).
dbg(Msg,Args)   -> log(debug(),Msg,Args).
